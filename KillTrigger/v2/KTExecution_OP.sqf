...
private ["_tName","_cvek","_victim","_timeToWait","_timeToWaitParam","_message","_destroyVeh","_time","_timeToKill","_prefix"];
		...
		if (_victim getVariable ["NSA_KT_SentInner",0] == -1) exitWith {
			...
			waitUntil  { 							//подождать пока а) игрок выйдет из триггера, б) выйдет время			
				...
				if (
					...
					|| 
					( !((getPosATL _victim select 2) <= 50) && !(((getPosATL _cvek select 2) <= 50) && (_victim in crew _cvek)) )
					||
...
if (_victim in vehicles) exitWith {
	[[[_message, _victim, _tName], {
		...
		waitUntil  { 		
			if ( !(_victim in list (_this select 2)) || !(player in crew _victim) || !(alive _victim) || !((getPosATL _victim select 2) <= 50) ) exitWith {true};
	...
	waitUntil  { 
		...
		if ( !(_victim in list _tName) || !((getPosATL _victim select 2) <= 50) || ( _timeToKill >= 1.0) ) exitWith { _victim setVariable ["NSA_KT_TimeToKill", _timeToKill, true]; true};
		sleep 0.5;
	};
